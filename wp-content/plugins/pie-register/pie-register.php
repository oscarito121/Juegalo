...
if( !class_exists('PieRegister') ){
	class PieRegister extends PieReg_Base{
			...
			*/
			...
			if( ( isset($_GET['prfrmid']) and ((int)$_GET['prfrmid']) != 0 and isset($_GET['action']) and $_GET['action'] == "delete" ) && $this->check_if_role_admin() )
			{
				...
				if(count($pr_all_form_info) > 1) {
					...
					$this->pie_post_array['error_message'] = __("Can not delete last form","piereg");
				}
			}
		...
		function pieActions(){
			...
			*/
			...
			if( current_user_can( 'administrator' ) && !$this->piereg_pro_is_activate )
				add_action( 'admin_notices', array( $this, 'pie_pro_version_release_notice' ), 15 );
		...
		*/
		public function pie_pro_version_release_notice()
		{
			$show_release_notice   = get_option( 'pie_pro_release_notice' );

			if ( empty( $show_release_notice ) ) {
				
				?>
				<div class="notice notice-warning pie-pro-release-notice is-dismissible"><p><?php echo sprintf( esc_html__( 'The much awaited Pie Register Pro has now been released. Purchase your license now from %s.', 'piereg' ), '<a target="_blank" href="http:///pieregister.com">pieregister.com</a>'); ?></p></div>
                <script type="text/javascript">
                	jQuery( '.pie-pro-release-notice' ).on( 'click', '.notice-dismiss', function() {		
						var data = {
							action: 'dismiss_pie_pro_release_notice'
						};
		...
		// @TODO remove this notice in the future.
		public function dismiss_pie_pro_release_notice()
		{
			update_option( 'pie_pro_release_notice', 'no' );
		}
...
?>
				...
				else
				{
					...
					if($error_found == 0){
						...
						if ( is_wp_error($user))
						{
							...
							{
								$user_login_error = apply_filters('pie_invalid_user_password_msg_txt','<strong>'.ucwords(__("error","piereg")).'</strong>: '.__("The password you entered for the username","piereg").' <strong>'.$this->pie_post_array['log'].'</strong> '.__("is incorrect","piereg").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","piereg").'">'.__("Lost your password?","piereg").'</a>');
							}
		...
		function saveFields()
		{
			...
			if(isset($_POST['piereg_reg_form_nonce']) && wp_verify_nonce( $_POST['piereg_reg_form_nonce'], 'piereg_wp_reg_form_nonce' ))
			{
				...
				$this->pie_post_array['error_message'] = __("Sorry, your nonce did not verify","piereg");
			}
		}
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				*/
				...
				else if($option_user_verification == 1 )//Admin Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration",__("Thank you for your registration. You will be notified once the admin approves your account.",'piereg'));
				}
				...
				else if($option_user_verification == 2 )//E-Mail Link Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration",__("Thank you for your registration. An activation link with your password has been sent to you.",'piereg'));
				...
				*/
				...
				{					
					$this->pie_post_array['registration_success'] = apply_filters("piereg_payment_sussess",__($wp_session['payment_sussess'],"piereg"));
					$wp_session['payment_error'] = "";
					$wp_session['payment_sussess'] = "";
				}
		...
		function PieRegSettingsProcess() {
			...
			if(isset($_POST['piereg_settings_allusers']) && wp_verify_nonce( $_POST['piereg_settings_allusers'], 'piereg_wp_settings_allusers' )) {
				...
				$this->pie_post_array['error'] = __("Sorry, your nonce did not verify","piereg");				
			}
		...
		*/
		...
		{ 
			...
			if( isset($_POST['piereg_renew_account_nonce']) && wp_verify_nonce( $_POST['piereg_renew_account_nonce'], 'piereg_wp_renew_account_nonce' ) )
			{
				...
				$this->pie_post_array['error'] = 'Sorry, your nonce did not verify';
			}
		}
			...
			if( isset($_POST['piereg_activate_addon_license_key'], $_POST['save_addon_license_key_settings']) )
			{
				if(isset($_POST['piereg_addon_license_key_nonce']))
				{
					if(wp_verify_nonce( $_POST['piereg_addon_license_key_nonce'], 'piereg_wp_addon_license_key_nonce' )){
						...
						//var_dump($piereg_pro_not_activated); die;
...
if( class_exists('PieRegister') ){
	...
	$GLOBALS['piereg_post_array'] 		= $pie_register->pie_post_array; 
